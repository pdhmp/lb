...
namespace NestCommDLL
{
    ...
    public class FIXConn : MessageCracker, QuickFix.Application , IDisposable
    {
        ...
        public string sendOrder(int ID_Ticker, int Qty, double price)
        {
            string result = sendOrder(ID_Ticker, Qty, price, "");
        ...
        {
            string SQLExpresion = "SELECT COD_REUTERS FROM NESTDB.DBO.TB119_CONVERT_SIMBOLOS WHERE ID_ATIVO = " + ID_Ticker.ToString();
        ...
        {
            if (DateTime.Today.Equals( NestCommDLL.Properties.Settings.Default.LastClOrdIDDate))
            {
                ordID = NestCommDLL.Properties.Settings.Default.LastClOrdID;
            }
            else
            {
                NestCommDLL.Properties.Settings.Default.LastClOrdIDDate = DateTime.Today;
                ordID = 0;
                NestCommDLL.Properties.Settings.Default.LastClOrdID = ordID;
                NestCommDLL.Properties.Settings.Default.Save();
            }
        }
        ...
        private void ExecutionFill(QuickFix42.ExecutionReport message, SessionID session)
        {
            ...
            if (allFieldsSet)
            {
                ...
                if (OrderContainer.ContainsKey(message.get(clOrdId).getValue()))
                {
                    ...
                    ExecutionFillArgs fillArgs = new ExecutionFillArgs(clOrdId.ToString(),
                                                                       OrderContainer[clOrdId.getValue()].ID_Ticker,
