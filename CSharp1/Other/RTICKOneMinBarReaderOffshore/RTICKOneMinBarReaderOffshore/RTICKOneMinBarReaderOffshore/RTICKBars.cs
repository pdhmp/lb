            ...
            foreach (string curFile in files)
            {
                using (newNestConn curConn = new newNestConn())
                {
                    ...
                    while ((tempLine = sr.ReadLine()) != null)
                    {
                        ...
                        {
                            if (double.Parse(curValues[2]) > 400)
                            {
                                StringSQL = StringSQL + "  "
                                            + "(" + IdSecurity.ToString() + ",'" + TempTicker + "','" + curValues[1] + "'," + curValues[2] + ",'" + curValues[3] + "','" + curValues[4] + "','"
                                            + curValues[5] + "','" + curValues[6] + "'," + curValues[7] + "," + curValues[8] + "," + curValues[9] + ", '" + DateTime.Now.ToString("yyyyMMdd") + "',0,0,0,0,0,0),";

                                InsertCounter++;
                                ControlCounter++;
                            }
                            if (InsertCounter >= 150)
                            {
                                string execSQL = "INSERT INTO NESTSRV02.RTICKDB.dbo.IntradayOneMinuteBars (IdTicker,Ticker,Date,Minute,VWAP,Volume,Low,High,[Open],[Close],TotalTrades,InsertDate,[StartTimeGMT],[StartTimeLocal],[EndTimeGMT],[EndTimeLocal],AskTrades,BidTrades) VALUES ";
                                Result = curConn.ExecuteNonQuery(execSQL + StringSQL.Substring(0, StringSQL.Length - 1));
