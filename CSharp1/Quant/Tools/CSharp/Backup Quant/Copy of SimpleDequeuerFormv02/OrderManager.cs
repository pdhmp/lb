...
namespace SimpleDequeuerForm
{
    public partial class OrderManager : Form
    {
        ...
        private BindingSource bndOrdSummary = new BindingSource();

        private BindingSource bndOrdersAux = new BindingSource();
        private BindingSource bndOrdSummaryAux = new BindingSource();

        private Dictionary<int, string> StrategiesDictionary = new Dictionary<int,string>();
        ...
        private void getStrategiesList()
        {
            ...
            {              
                idsec = Int32.Parse(sectionsTable.Rows[i][0].ToString());
                sec = (string)sectionsTable.Rows[i][1];
                StrategiesDictionary.Add(idsec,sec);
            }
        ...
        private void doConnection()
        {
            ...
            dgcOrdersSummary.DataSource = bndOrdSummary;
        ...
        private void ReceiveTuple(Tuple tuple)
        {
            ...
            {               
                if (tuple.GetField(AppOrdID) != null && tuple.GetField(Symbol) != null)
                {
                    ...
                    addOrUpdateOrders(_orderId, _symbol, _side, _orderStatus, _avgPrice, _orderQty, _lastShares, _leavesQty, _updateTime, _OrdRejReason);
                }
            }
        ...
        private void addOrUpdateOrders(string _OrderId, string _Symbol, string _Side, string _OrderStatus, double _AveragePrice, double _OrderQuantity, double _Executed, double _Leaves, string _UpdateTime, string _ExtraInfo)
        {
            ...
            foreach (OrderManagerContainter omc in bndOrders)
            {
            ...
            {                
                ...
                if (getStrategy(_OrderId) != "")
                {
                    szAux = Int32.Parse(getStrategy(_OrderId));
                }
                ...
                bndOrders.Add(new OrderManagerContainter(_OrderId, _Strategy, _Symbol, _Side, _OrderStatus, _AveragePrice, _OrderQuantity, _Executed, _Leaves, _UpdateTime, _ExtraInfo));
        ...
        private string getStrategy(string OrderId)
        {
        ...
        private void rdShowAll_CheckedChanged(object sender, EventArgs e)
        {
            if (rdShowAll.Checked)
            {
                ...
                dgcOrders.DataSource = bndOrders;
                bndOrdSummary.Clear();
                string modeView = cbOptions.Text;
                foreach (OrderManagerContainter omc in bndOrders)
                {
                    if (String.Compare(omc.OrderStatus, modeView) == 0)
                    {
                        bndOrdSummary.Add(omc);
                    }
                }
                dgcOrdersSummary.DataSource = bndOrdSummary;
            }            
        ...
        private void rdShowStrategy_CheckedChanged(object sender, EventArgs e)
        {
            if (rdShowStrategy.Enabled)
            {
                ...
                foreach (OrderManagerContainter omc in bndOrders)
                {
                    ...
                    {
                        bndOrdersAux.Add(omc);
                    }
                }

                dgcOrders.DataSource = bndOrdersAux;
                string modeView = cbOptions.Text;
                bndOrdSummaryAux.Clear();
                foreach (OrderManagerContainter omc in bndOrdersAux)
                {
                    if (String.Compare(modeView, omc.OrderStatus) == 0)
                    {
                        bndOrdSummaryAux.Add(omc);
                    }
                }
                dgcOrdersSummary.DataSource = bndOrdSummaryAux;
            }
        }


        private void OrderManager_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void cbOptions_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (rdShowStrategy.Enabled)
            {
                string modeView = cbOptions.Text;
                bndOrdSummaryAux.Clear();
                foreach (OrderManagerContainter omc in bndOrdersAux)
        ...
        private void cbStrategies_SelectedIndexChanged(object sender, EventArgs e)
        {
            string strategy = cbStrategies.Text;
            bndOrdersAux.Clear();
            foreach (OrderManagerContainter omc in bndOrders)
        ...
        private void dgcOrders_Click(object sender, EventArgs e)
        {
            
            System.Data.DataRow d= dgvOrders.GetDataRow(dgvOrders.GetFocusedDataSourceRowIndex());         
        }           
    }
}
