...
namespace ReutersTickOneMinBarsGenerator
{
    public partial class Form1 : Form
    {
        ...
        private void Run(object obj)
        {
            ...
            bool gohistorical = true;

            if (gohistorical == true)
            {

                DateTime date = new DateTime(2013, 08, 01);
                QSEG.CalculateHistoricalIndexes(date);

            }
            else
            {

                DateTime date = DateTime.Today.DayOfWeek == DayOfWeek.Monday ? DateTime.Today.AddDays(-3) : DateTime.Today.AddDays(-4);
                ...
                while ((tempLine = sr.ReadLine()) != null)
                {
                    ...
                    if (OpenTime != new TimeSpan(0, 0, 0))
                    {
                        if (curType == "Trade" && curState != "ODD")
                        {
                            ...
                            if (curTime > new TimeSpan(CloseTime.Hours, CloseTime.Minutes, 0).Add(new TimeSpan(0, 0, -5)) && OpenAuctionEnded)
                            {
        ...
        private void LoadOpenTimes(int Id_Mercado)
        {
            ...
            */
            ...
            foreach (DateTime Date in OpenDates)
            {
                ...
                string OpenTimeQuery;

                if (Date > new DateTime(2006, 01, 01))
                {
                    OpenTimeQuery = "SELECT TOP 1 OpenTimeLocal,CloseTimeLocal,GMT_Offset " +
                    "FROM NESTDB.dbo.Tb108_Horarios_Mercados " +
                    "WHERE DateRef <= '" + Date.ToString("yyyy-MM-dd") + "' AND Id_Mercado = " + Id_Mercado + " " +
                    "ORDER BY DateRef DESC ";
                }
                else
                {
                    OpenTimeQuery = "SELECT TOP 1 OpenTimeLocal,CloseTimeLocal,GMT_Offset " +
                    "FROM NESTDB.dbo.Tb108_Horarios_Mercados " +
                    "WHERE DateRef <= '2006-01-01' AND Id_Mercado = " + Id_Mercado + " " +
                    "ORDER BY DateRef DESC ";
                    ...
                    using (newNestConn curConn = new newNestConn(true))
                    {
                        ...
                        while ((tempLine = sr.ReadLine()) != null)
                        {
                            ...
                            if (!double.IsNaN(IdSecurity))
                            {
                                ...
                                for (int i = 2; i < 4; i++)
                                {
                                    if (curValues[i].Contains("-"))
                                    {
                                        curValues[i] = curValues[i].Replace("-", "");
                                    }
                                }
                                ...
                                try
                                {
                                    ...
                                    StringSQL = "INSERT INTO RTICKDB.dbo.OpenAuction (IdTicker,Ticker,Date,EndTimeGMT,EndTimeLocal,Price,Volume,TotalTrades,InsertDate) VALUES "
                                            + "(" + IdSecurity.ToString() + ",'" + TempTicker + "','" + curValues[1] + "','" + curValues[2] + "','" + curValues[3] + "'," + curValues[4] + "," + int.Parse(curValues[5].ToString()) + "," + curValues[6] + ",'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:00.000") + "')";

                                    Result = curConn.ExecuteNonQuery(StringSQL);
                                }
                                catch (SqlException rr)
                                {
                                    Console.WriteLine(rr.Message.ToString());
                                }
                                catch (StackOverflowException xx)
                                {
                                    Console.WriteLine(xx.Message.ToString());
                                }
                                catch (Exception jj)
                                {
                                    Console.WriteLine(jj.Message.ToString());
                                }
                            ...
                            if (!double.IsNaN(IdSecurity))
                            {
                                ...
                                try
                                {
                                    ...
                                    StringSQL = "INSERT INTO RTICKDB.dbo.CloseAuction (IdTicker,Ticker,Date,StartTimeGMT,StartTimeLocal,EndTimeGMT,EndTimeLocal,PreAuction_Price,Price,Volume,TotalTrades,InsertDate) VALUES "
                                                + "(" + IdSecurity.ToString() + ",'" + TempTicker + "','" + curValues[1] + "','" + curValues[2] + "','" + curValues[3] + "','" + curValues[4] + "','" + curValues[5] + "'," + curValues[6] + "," + curValues[7] + "," + curValues[8] + "," + curValues[9] + ",'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:00.000") + "')";
                            ...
                            if (!double.IsNaN(IdSecurity))
                            {
                                ...
                                try
                                {
                                    StringSQL = "INSERT INTO RTICKDB.dbo.IntradayOneMinuteBars (IdTicker,Ticker,Date,Minute,StartTimeGMT,StartTimeLocal,EndTimeGMT,EndTimeLocal,VWAP,Volume,Low,High,[Open],[Close],AskTrades,BidTrades,TotalTrades,InsertDate,RegressiveMinute) VALUES "//,MinSpread,MaxSpread,AverageSpread) VALUES "
                                                ...
                                                + "," + curValues[11] + "," + curValues[12] + "," + curValues[13] + "," + curValues[14] + "," + curValues[15] + "," + curValues[16] + ",'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:00.000") + "'," + curValues[17] + ")"; //+ "," + curValues[18] + "," + curValues[19] + "," + curValues[20] + ")";
