...
namespace MarketClusters
{
    ...
    class Utils
    {
        public static string GetTickerSymbol(int Id_Ticker)
        {
            ...
            {
                string SQLString = "SELECT Simbolo + '(' + LEFT(UPPER(Setor), 4) + ')' FROM NESTDB.dbo.Tb001_Ativos A LEFT JOIN dbo.Tb000_Instituicoes B ON A.Id_Instituicao=B.Id_Instituicao LEFT JOIN dbo.Tb113_Setores C ON B.Id_Setor=C.Id_Setor WHERE Id_Ativo=" + Id_Ticker.ToString();
        ...
        public static PointPairList[] LoadArray(int Id_Ticker, DateTime iniDate, DateTime endDate)
        {
            ...
            using (NestConn conn = new NestConn())
            {
                string SQLString = "SELECT Data_Hora_Reg, Valor FROM NESTDB.dbo.Tb050_Preco_Acoes_Onshore WHERE Id_Ativo=" + Id_Ticker + " AND Tipo_Preco=100 AND Source=1 AND Data_Hora_Reg>='" + iniDate.ToString("yyyy-MM-dd") + "' AND Data_Hora_Reg<='" + endDate.ToString("yyyy-MM-dd") + "' ORDER BY Data_Hora_Reg";
