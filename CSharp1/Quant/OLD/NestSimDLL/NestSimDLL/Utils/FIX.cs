...
namespace NestQuant.Common
{
    ...
    public class FIXConn : MessageCracker, QuickFix.Application , IDisposable
    {
        ...
        {            
            ...
            Symbol _symbol = new Symbol(getSymbol(ID_Ticker));
            Side _side = new Side((Qty >= 0 ? Side.BUY : Side.SELL));            
            TransactTime _tTime = new TransactTime(DateTime.Now);
            OrdType _ordType = new OrdType((price > 0 ? OrdType.LIMIT : OrdType.MARKET));            
        ...
        {
            if (DateTime.Today.Equals( NestQuant.Common.Properties.Settings.Default.LastClOrdIDDate))
            {
                ordID = NestQuant.Common.Properties.Settings.Default.LastClOrdID;
            }
            else
            {
                NestQuant.Common.Properties.Settings.Default.LastClOrdIDDate = DateTime.Today;
                ordID = 0;
                NestQuant.Common.Properties.Settings.Default.LastClOrdID = ordID;
                NestQuant.Common.Properties.Settings.Default.Save();
            }
        }
        ...
        private void ExecutionFill(QuickFix42.ExecutionReport message, SessionID session)
        {
            ...
            if (allFieldsSet)
            {
                ...
                if (OrderContainer.ContainsKey(message.get(clOrdId).getValue()))
                {
                    ...
                    ExecutionFillArgs fillArgs = new ExecutionFillArgs(OrderContainer[clOrdId.getValue()].ID_Ticker,
