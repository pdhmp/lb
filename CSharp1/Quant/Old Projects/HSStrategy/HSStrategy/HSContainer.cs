...
namespace NestQuant.Strategies
{
    public class HSContainer
    {
        ...
        public Price_Table SignalTables;
        ...
        public HSContainer(int _Id_SimSet, bool _Intraday, int _Id_Ticker_Composite,
                           ...
                           DateTime _BeginDate, DateTime _EndDate)
        {
            ...
            EndDate = _EndDate;
            Id_Ticker_List = getTickers(_Id_Ticker_Composite);
            Intraday = _Intraday;
        ...
        private int[] getTickers(int _Id_Ticker_Composite)
        {
            int[] result = new int[500];

            string SQLExpression = "select id_ticker_component from tb023_securities_composition where id_ticker_composite = " + _Id_Ticker_Composite + " group by id_ticker_component";
            ...
            using (NestConn conn = new NestConn())
            {
                ...
                conn.openConn();
                SqlDataReader response = conn.ExecuteReader("select count(*) from ("+SQLExpression+") as a");

                while (response.Read())
                {
                    result = new int[response.GetInt32(0)];
                }
            }
            ...
            {
                conn.openConn();
                SqlDataReader response = conn.ExecuteReader(SQLExpression);                            
                int i = 0;
                while(response.Read())
                {
                    result[i] = response.GetInt32(0);
                    i++;
                }                
            }
