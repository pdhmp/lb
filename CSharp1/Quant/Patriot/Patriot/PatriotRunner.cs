...
namespace Patriot
{
    public class PatriotRunner
    {
        ...
        private double _Last30Perc = 0.1; //0.2; //30%
        ...
        private double _CloseAucPerf = 0.075; //0.15; //20%
        ...
        public void LoadPairs()
        {
            ...
            using (newNestConn curConn = new newNestConn())
            {
                ...
                string SQLExpression = " SELECT IDSECURITY,NESTTICKER FROM NESTDB.DBO.TB001_SECURITIES" +
                                       ...
                                       " 1231,1687,809,381,382,383,384,390,393,415,80873,1754,1074,448," +
        ...
        private void GenerateBasket()
        {
            ...
            SortedDictionary<double, double> auxBuyOrders = new SortedDictionary<double, double>();
            SortedDictionary<double, double> auxSellOrders = new SortedDictionary<double, double>();
            ...
            foreach (TradingPairs curPair in toTrade)
            {
                ...
                if (auxSellOrders.TryGetValue(curPair.Security1, out sellSec1))
                {
                    auxSellOrders[curPair.Security1] -= Math.Min(sellSec1, sec1Net);
                    sec1Net-= Math.Min(sellSec1,sec1Net);

                    if (auxSellOrders[curPair.Security1] == 0)
                    {
                        auxSellOrders.Remove(curPair.Security1);
                    }                    
                }

                if (sec1Net > 0)
                {
                    if (auxBuyOrders.ContainsKey(curPair.Security1))
                    {
                        auxBuyOrders[curPair.Security1] += sec1Net;
                    }
                    else
                    {
                        auxBuyOrders.Add(curPair.Security1, sec1Net);                        
                    }
                }

                if (auxBuyOrders.TryGetValue(curPair.Security2, out buySec2))
                {
                    auxBuyOrders[curPair.Security2] -= Math.Min(buySec2, sec2Net);
                    sec2Net -= Math.Min(buySec2, sec2Net);

                    if (auxBuyOrders[curPair.Security2] == 0)
                    {
                        auxBuyOrders.Remove(curPair.Security2);
                    }
                }

                if (sec2Net > 0)
                {
                    if (auxSellOrders.ContainsKey(curPair.Security2))
                    {
                        auxSellOrders[curPair.Security2] += sec2Net;
                    }
                    else
                    {
                        auxSellOrders.Add(curPair.Security2, sec2Net);                       
                    }
                }                             
            }

            SortedDictionary<double, double> M1auxBuy = new SortedDictionary<double, double>();
            SortedDictionary<double, double> M1auxSell = new SortedDictionary<double, double>();

            double OrigGross = 0;
            double AdjGross = 0;

            foreach (KeyValuePair<double, double> kvp1 in auxBuyOrders)
            {
                double F330avg = PatriotData.Instance.First330Avg5d.GetValue(kvp1.Key, refDate, true)[1];
                double F330 = PatriotData.Instance.First330Vwap.GetValue(kvp1.Key, curDate, true)[1] * PatriotData.Instance.First330Volume.GetValue(kvp1.Key, curDate, true)[1];
                double M1 = Math.Min(1.0, F330 / F330avg);

                double adjFinan = kvp1.Value * M1;

                OrigGross += kvp1.Value;
                AdjGross += adjFinan;

                M1auxBuy.Add(kvp1.Key, adjFinan);                
            }
            foreach (KeyValuePair<double, double> kvp1 in auxSellOrders)
            {
                double F330avg = PatriotData.Instance.First330Avg5d.GetValue(kvp1.Key, refDate, true)[1];
                double F330 = PatriotData.Instance.First330Vwap.GetValue(kvp1.Key, curDate, true)[1] * PatriotData.Instance.First330Volume.GetValue(kvp1.Key, curDate, true)[1];
                double M1 = Math.Min(1.0, F330 / F330avg);
                ...
                M1auxSell.Add(kvp1.Key, adjFinan);
            }

            double adjFactor = AdjGross / OrigGross;
            //double adjFactor = 1;
            ...
            foreach (KeyValuePair<double, double> kvp1 in auxBuyOrders)
            {
                BuyOrders.Add(kvp1.Key, kvp1.Value * adjFactor);
            }
            foreach (KeyValuePair<double, double> kvp1 in auxSellOrders)
            {
                SellOrders.Add(kvp1.Key, kvp1.Value * adjFactor);
            }
        ...
        private void GeneratePOVOutputFile()
        {
            StreamWriter swPovOrders = new StreamWriter(@"C:\Quant\Strategies\Patriot\Patriot_ORDERS_POV_" + curDate.ToString("yyyyMMdd") + ".csv", false);            

            foreach (KeyValuePair<double, double> kvp in BuyOrders)
            {
                double avg5dL30 = PatriotData.Instance.GetValue(kvp.Key, 332, curDate, curDate, false)[1];
                double avg5dAuc = PatriotData.Instance.GetValue(kvp.Key, 313, curDate, curDate, false)[1];
                double vwapF330 = PatriotData.Instance.GetValue(kvp.Key, 351, curDate, curDate, false)[1];

                double maxShares = kvp.Value / vwapF330;
                maxShares = Math.Round(maxShares / 100F, 0) * 100F;

                double L30Perc = kvp.Value / avg5dL30;
                double AucPerc = kvp.Value / avg5dAuc;                

                double tickerPerc = Math.Max(L30Perc, AucPerc)*100;

                string ticker = IdSecurityToTicker[kvp.Key];

                string line = "";

                line = line + ticker + ";";
                line = line + "1" + ";";
                line = line + maxShares.ToString() + ";";
                line = line + kvp.Key.ToString() + ";";
                line = line + avg5dL30.ToString() + ";";
                line = line + curDate.ToString("dd/MM/yyyy") + ";";
                line = line + vwapF330.ToString() + ";";
                line = line + kvp.Value.ToString() + ";";
                line = line + kvp.Value.ToString() + ";";
                line = line + maxShares.ToString() + ";";                
                line = line + tickerPerc.ToString() + ";";
                line = line + "" + ";";
                line = line + kvp.Value.ToString() + ";";
                

                swPovOrders.WriteLine(line);
            }
            foreach (KeyValuePair<double, double> kvp in SellOrders)
            {
                double avg5dL30 = PatriotData.Instance.GetValue(kvp.Key, 332, curDate, curDate, false)[1];
