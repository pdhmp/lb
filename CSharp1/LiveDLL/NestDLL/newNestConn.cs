...
namespace LiveDLL
{
    public class newNestConn : IDisposable
    {
        ...
        private void InitConn(bool UseOldConn1)
        {
            this.UseOldConn = UseOldConn1;

            if (!this.UseOldConn)
            {
                if (ConnManager.Instance.ConexaoDB == null)
                    ConnManager.Instance.OpenConnection(UseOldConn1);
            }
            else
            {
                if (NestSrv02Conn.Instance.ConexaoDB == null)
                    NestSrv02Conn.Instance.OpenConnection();
            }
        }
