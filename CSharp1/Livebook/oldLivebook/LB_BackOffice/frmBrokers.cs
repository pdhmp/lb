...
namespace LiveBook
{
    public partial class frmBrokers : LBForm
    {
        ...
        void LoadCombos()
        {
            NestDLL.FormUtils.LoadCombo(this.cmbd_Id_praca, "Select Id_Geografia,Nome from Tb100_Geografia order by ordem", "Id_Geografia", "Nome", 99);
        }
        ...
        void ReloadlstAvDates()
        {
            using (newNestConn curConn = new newNestConn())
            {
                ...
                NestDLL.FormUtils.LoadList(lstAvDates, "SELECT ValidAsOfDate FROM NESTDB.dbo.Tb012_Brokers_Variable (nolock) WHERE IdBroker=" + lbld_Id_Corretora.Text, "ValidAsOfDate", "ValidAsOfDate");
                this.lstAvDates.SelectedIndexChanged += new System.EventHandler(this.lstAvDates_SelectedIndexChanged);
