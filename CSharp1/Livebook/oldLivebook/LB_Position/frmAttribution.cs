    ...
    public partial class frmAttribution : LBForm
    {
        ...
        public void Carrega_Grid()
        {
            ...
            using (newNestConn curConn = new newNestConn())
            {
                ...
                if (runType == 1)
                {
                    curConn.ExecuteNonQuery("INSERT INTO #tempResultAtt EXEC [dbo].[Proc_Attribution] " + Id_Portfolio + ", " + viewBooks + ", '" + iniDate.ToString("yyyyMMdd") + "', '" + endDate.ToString("yyyyMMdd") + "', " + flagCumulative + "");
                ...
                {
                    SQLFields = SQLFields + ",COALESCE(SUM(CASE WHEN [Date Now]='" + Convert.ToDateTime(row[0]).ToString("yyyyMMdd") + "' THEN " + getField + " ELSE 0 END), 0) AS '" + Convert.ToDateTime(row[0]).ToString("dd/MMM/yy") + "'";
                }
