...
namespace SGN
{
    public partial class frmcoluns : System.Windows.Forms.Form
    {
        ...
        private void cmdOk_Click(object sender, EventArgs e)
        {
            ...
            //string SQLString = "Select Nome_Coluna from where Id_Usuario =" + Id_User + " and Grid_Cod = " + this.cmdgrid.SelectedValue;

            string SQLString = " Select Nome_Coluna from Tb109_Caractisticas_Colunas" +
                               " where Visible=0 and Campo_Id= 0" +
                               " and Id_User= " + Id_User + " and Id_Grid= " + this.cmdgrid.SelectedValue + " order by Indice";
            
            SqlDataReader dr1 = CargaDados.curConn.Return_DataReader(SQLString);
            ...
            SQLString = "Select Nome_Coluna from Tb109_Ordens_Colunas where Id_Usuario =" + Id_User + " and Grid_Cod = " + this.cmdgrid.SelectedValue;

             SQLString = " Select Nome_Coluna from Tb109_Caractisticas_Colunas" +
                   " where Visible=1 and Campo_Id= 0" +
                   " and Id_User= " + Id_User + " and Id_Grid= " + this.cmdgrid.SelectedValue + " order by Indice";

            SqlDataReader dr2 = CargaDados.curConn.Return_DataReader(SQLString);
        ...
        private void cmdSave_Click(object sender, EventArgs e)
        {
            ...
            while (F < contador)
            {
                ...
                SQLString = "update Tb109_Caractisticas_Colunas set Visible = 1 where  Nome_Coluna='" +  lst2.Items[F].ToString() + "' and Id_User= " + Id_User + " and Id_Grid= " + Id_Grid;
                zzz = CargaDados.curConn.ExecuteNonQuery(SQLString,1);
                if (zzz == 0)
                {
            ...
            while (F < contador)
            {
                ...
                SQLString = "update Tb109_Caractisticas_Colunas set Visible = 0 where  Nome_Coluna='" +  lst1.Items[F].ToString() + "' and Id_User= " + Id_User + " and Id_Grid= " + Id_Grid;
                zzz = CargaDados.curConn.ExecuteNonQuery(SQLString,1);
                if (zzz == 0)
                {
                    MessageBox.Show("Error in Insert!");
