...
namespace LiveBook
{
    public class NestBookCombo : System.Windows.Forms.ComboBox
    {
        ...
        public void ChangePortfolio()
        {
            ...
            {
                NestDLL.FormUtils.LoadCombo(this, "SELECT -1 AS Id_Book, 'All Books' AS Book UNION SELECT [Id Book] AS Id_Book, Book from NESTRT.dbo.Tb000_Posicao_Atual (nolock) WHERE [Id Portfolio]=" + _Id_Portfolio + " GROUP BY [Id Book], Book", "Id_Book", "Book", curValue);
            }
            else
            {
                NestDLL.FormUtils.LoadCombo(this, "SELECT [Id Book] AS Id_Book, Book from NESTDB.dbo.Tb000_Historical_Positions (nolock) WHERE [Id Portfolio]=" + _Id_Portfolio + " GROUP BY [Id Book], Book", "Id_Book", "Book", curValue);
            }
        }
    }
}
