...
namespace LiveBook
{
    public partial class frmEditLimitsArb : LBForm
    {
        ...
        {
            NestDLL.FormUtils.LoadCombo(cmbd_StressType, "SELECT IdStressType,StressType FROM NESTDB.dbo.Tb821_StressType (nolock)", "IdStressType", "StressType");

            NestDLL.FormUtils.LoadCombo(cmbPar2, "SELECT IdSecurity,NestTicker FROM NESTDB.dbo.Tb001_Securities (nolock) WHERE IdInstrument NOT IN (3, 11, 12) ORDER BY NestTicker", "IdSecurity", "NestTicker");
            NestDLL.FormUtils.LoadCombo(cmbPar3, "SELECT IdSecurity,NestTicker FROM NESTDB.dbo.Tb001_Securities (nolock) WHERE IdInstrument NOT IN (3, 11, 12) ORDER BY NestTicker", "IdSecurity", "NestTicker");
         ...
         void ReloadlstAvDates()
        {
            ...
            using (newNestConn curConn = new newNestConn())
            {
                ...
                NestDLL.FormUtils.LoadList(lstAvDates, "SELECT ValidDate,ValidDate AS Valid2 FROM NESTDB.dbo.Tb820_ArbStress (nolock) WHERE IdSection=" + IdSection + " ORDER BY ValidDate", "ValidDate", "ValidDate");
                this.lstAvDates.SelectedIndexChanged += new System.EventHandler(this.lstAvDates_SelectedIndexChanged);
       ...
       void LoadStress(int IdSection, DateTime ValidDate)
        {
            ...
            using (newNestConn curConn = new newNestConn())
            {
                ...
                foreach (DataRow curRow in tablep.Rows)
                {
                   ...
                   dtpEndDate.Value = NestDLL.Utils.ParseToDateTime(curRow["EndDate"]);
                   dtpIniDate.Value = NestDLL.Utils.ParseToDateTime(curRow["IniDate"]);
