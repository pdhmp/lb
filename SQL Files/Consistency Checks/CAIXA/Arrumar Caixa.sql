...
FROM 
(
	SELECT [Date Now] AS TrDate, MAX([Last Position]) as TrLastPos, MAX([Close_Date]) as TrCloseDate, SUM(Position) AS Hist_PositionLB, MAX([NAV]) as TrNAV, MAX([NAV pC]) AS TrNAVpC
...
WHERE [Id Portfolio]=10 AND [Id Ticker]=1844 AND [Date Now]='2010-09-27' 
