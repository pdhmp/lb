...
namespace LiveBook
{
    public partial class frmStrategyTransfer : LBForm
    {
        ...
        private void frmInsertOrder_Load(object sender, EventArgs e)
        {
            ...
            LiveDLL.FormUtils.LoadCombo(this.cmbBookFrom, "Select Id_Book, Book from dbo.Tb400_Books (nolock) ", "Id_Book", "Book", 99);
            Carrega_Section1();
            LiveDLL.FormUtils.LoadCombo(this.cmbBookTo, "Select Id_Book, Book from dbo.Tb400_Books (nolock) ", "Id_Book", "Book", 99);
            Carrega_Section2();
        ...
        public void Carrega_Portfolio()
        {
            try
            {
                LiveDLL.FormUtils.LoadCombo(this.cmbportfolio, "Select Id_Portfolio,Port_Name FROM dbo.Tb002_Portfolios (nolock) Where Discountinued <> 1 and Id_Port_Type =2", "Id_Portfolio", "Port_Name", 99);
            }
            catch (Exception e)
            {
        ...
        private void cmbportfolio_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbportfolio.SelectedValue != null)
            {
                ...
                //LiveDLL.FormUtils.LoadCombo(this.cmbTicker, StringSQL, "IdSecurity", "NestTicker", 99);
        ...
        public void Carrega_Section1()
        {
            ...
            try
            {
                if (curUtils.IsNumeric(cmbBookFrom.SelectedValue))
                {
                    ...
                    //SubEstrategia
                    LiveDLL.FormUtils.LoadCombo(this.cmbSectionFrom, "Select Id_Section,Section from VW_Book_Strategies where Id_Book =" + valor, "Id_Section", "Section", 99);
                }
            }
            catch (Exception e)
            {
        ...
        public void Carrega_Section2()
        {
            ...
            try
            {
                if (curUtils.IsNumeric(cmbBookTo.SelectedValue))
                {
                    ...
                    //SubEstrategia
                    LiveDLL.FormUtils.LoadCombo(this.cmbSectionTo, "Select Id_Section,Section from VW_Book_Strategies where Id_Book =" + valor, "Id_Section", "Section", 99);
                }
            }
            catch (Exception e)
            {
        ...
        int TransferPosition()
        {
            ...
            int IdBookFrom = Convert.ToInt32(cmbBookFrom.SelectedValue.ToString());
            int IdSectionFrom = Convert.ToInt32(cmbSectionFrom.SelectedValue.ToString());
            int IdBookTo = Convert.ToInt32(cmbBookTo.SelectedValue.ToString());
            int IdSectionTo = Convert.ToInt32(cmbSectionTo.SelectedValue.ToString());
            ...
            decimal Cash = Price * (Quantity / RoundLot);
            ...
            StringFrom = "INSERT INTO [NESTDB].[dbo].[Tb700_Transactions] " +
                        "(Transaction_Type,[Trade_Date],Settlement_Date,Id_Account1,Id_Ticker1,[Id Book1],[Id Section1],Quantity1,Id_Account2,Id_Ticker2,[Id Book2],[Id Section2],Quantity2) " +
                        "VALUES (11,'" + dtpDate.Value.ToString("yyyyMMdd") + "','" + dtpDate.Value.ToString("yyyyMMdd") + "'," + Id_Account.ToString() + "," + IdSecurity.ToString() + "," + IdBookFrom.ToString() + "," + IdSectionFrom.ToString() + "," + Quantity.ToString().Replace(".", "").Replace(",", ".") + "," + Id_Account.ToString() + "," + IdCash + ",5,1," + Cash.ToString().Replace(".", "").Replace(",", ".") + ") ;  " +
                        "Select @@IDENTITY ";

            int retorno = curConn.Return_Int(StringFrom);

            
            Cash = Cash * -1;
            Quantity = Quantity * -1;

            StringTo = " INSERT INTO [NESTDB].[dbo].[Tb700_Transactions](Transaction_Type,[Trade_Date],Settlement_Date,Id_Account1,Id_Ticker1,[Id Book1],[Id Section1],Quantity1,Id_Account2,Id_Ticker2,[Id Book2],[Id Section2],Quantity2,IdRelation) " +
            ...
            retorno = curConn.ExecuteNonQuery(StringTo);
