...
namespace LiveBook
{
    public partial class frmOrderReview : LBForm
    {
        ...
        private void LoadOrders()
        {
            using (newNestConn curConn = new newNestConn())
            {
                string SQLString = "SELECT * " +
                            ...
                            " ORDER BY A.[Id Order], A.[Id Portfolio]";
                ...
                foreach (DataRow curRow in curTable.Rows)
                {
                    //if (curOrder.IdOrder == 0)
                    {
                        ...
                        {
                            IdOrder = Utils.ParseToDouble(curRow["Id Order"]),
                            Portfolio = curRow["Portfolio"].ToString(),
                            Book = curRow["Book"].ToString(),
                            Section = curRow["Section"].ToString(),
                            Ticker = curRow["Ticker"].ToString(),
                            Total = Utils.ParseToDouble(curRow["Total"]),
                            Done = Utils.ParseToDouble(curRow["Done"]),
                            Leaves = Utils.ParseToDouble(curRow["Leaves"]),
                            CashDone = Utils.ParseToDouble(curRow["Cash Done"]),
                            AvgPriceDone = Utils.ParseToDouble(curRow["Avg Price Done"]),
                            Login = curRow["Login"].ToString(),
                            Status = curRow["Status"].ToString(),
                            Split = (int)Utils.ParseToDouble(curRow["Split"]),
                            OrderDate = Utils.ParseToDateTime(curRow["Order Date"]),
                            Currency = curRow["Currency"].ToString(),
                            Broker = curRow["Broker"].ToString(),
                            IdBroker = (int)Utils.ParseToDouble(curRow["Id Broker"]),
                            IdBook = (int)Utils.ParseToDouble(curRow["Id Book"]),
                            IdSection = (int)Utils.ParseToDouble(curRow["Id_Section"]),
                            OrderPrice = Utils.ParseToDouble(curRow["Order Price"])
                        };
                        OrderList.Add(curOrder);
                    }
                }

                SQLString = "SELECT * FROM VW_LB_Trades WHERE [Trade Date]='" + DateTime.Now.ToString("yyyy-MM-dd") + "' AND Id_Port_Type = 2 ORDER BY TimeStamp ";
                ...
                foreach (DataRow curRow in curTable.Rows)
                {
                    if (Utils.ParseToDouble(curRow["Trade Quantity"]) != 0)
                    {
                        ...
                        curTrade.IdOrder = (int)Utils.ParseToDouble(curRow["Id Order"]);
                        ...
                        foreach (Order TradeOrder in OrderList)
                        {
                            if (TradeOrder.IdOrder == curTrade.IdOrder)
                            {
                                TradeOrder.AddTrade(curTrade);
                            }
                        }
                    }
                }
        ...
        private void dgOrderReview_DoubleClick(object sender, EventArgs e)
        {
            ...
            lblAtualizar.Text = LinhasNaoAtualizadas.ToString();
        }
        ...
        private void dgTrades_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (Column_Name == "Edit" && Id_Status != 4)
            {
