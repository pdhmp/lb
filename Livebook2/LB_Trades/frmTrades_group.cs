    ...
    public partial class frmTrades_group : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            dtgTrade.LookAndFeel.SetSkinStyle("Blue");
                ...
                LiveDLL.FormUtils.LoadCombo(this.cmbChoosePortfolio, "Select Id_Portfolio,Port_Name from  dbo.Tb002_Portfolios where Id_Port_Type in (1,2)", "Id_Portfolio", "Port_Name", 1);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            string SQLString = @"   Select	 [Trade Date] AS OperationDate
		                                    ...
		                                    ,'C SUISSE' AS Broker
		                                    ,ROUND(ABS(SUM([Trade Quantity])),0) AS Qty
		                                    ,ROUND((SUM([Trade Price]*[Trade Quantity])/(CASE WHEN SUM([Trade Quantity])=0 THEN 1 ELSE SUM([Trade Quantity]) END)) * (CASE [IdInstrument] WHEN 3 THEN 1 ELSE 1 END), 6) AS [Price] 
                                            ,SymbolType
                                    FROM	( 
            ...
            // ,ROUND((SUM([Trade Price]*[Trade Quantity])/(CASE WHEN SUM([Trade Quantity])=0 THEN 1 ELSE SUM([Trade Quantity]) END)) * (CASE [IdInstrument] WHEN 3 THEN 100 ELSE 1 END), 6) AS [Price] 
        ...
        protected void ExportExcel(DataTable table, string filename)
        {
            ...
            {
                MessageBox.Show("Failed to create Excel output filed : " + e, "LiveBook", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
