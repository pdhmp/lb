    ...
    public partial class frmImportDividends : Form
    {
        ...
        private void ReadWebPage()
        {
            ...
            while((pos = result.IndexOf("\" target=\"Noticia\">PROVENTOS", pos + 1)) != -1)
            {
                ...
                double retorno = LiveDLL.Utils.ParseToDouble(curConn.Execute_Query_String(StringSQL));
                string flagStatus;
                if(retorno > 0)
                {
                    flagStatus = "Delete";
        ...
        private void ReadNews(string urlnews, string horario, string Id_News)
        {
            ...
            // string url = "http://www.bmfbovespa.com.br/Agencia/corpo.asp?origem=exibir&id=18201305240011&manchete=PROVENTOS%20DAS%20EMPRESAS%20COTADAS%20NA%20FORMA%20&quot;EX&quot;:%2024/05/2013%22";

            string result = client.DownloadString(url);

            int pos = result.IndexOf("EX&quot;: ", 1);

            string Data = result.Substring(pos + 10, 10);
        ...
        private int ReadFile(string curText, DateTime Data, string horario, string Id_News)
        {
            ...
            while((pos = curText.IndexOf("APROVADO", pos + 1)) != -1)
            {
                ...
                Ticker = curText.Substring(pos - 36, 13).Trim();
                ...
                if (TipoProvento == "SUBSCRICAO")
                {
                    int pos2 = curText.IndexOf("PERCENT:", pos + 1);
                    string sPercent = curText.Substring(pos2 + 8, 12).Trim();

                    int pos3 = curText.IndexOf("VLR EMIS:", pos + 1);
                    valor = curText.Substring(pos3 + 9, 12).Trim();

                    PercAdjust = 100;
                    FlagInsert = true;
                    TransType = 23;

                    int resposta = Convert.ToInt32(MessageBox.Show("Confirm update ?", "Insert Subscription", MessageBoxButtons.YesNo, MessageBoxIcon.Question));
                    if (resposta == 6)
                    {
                        frmSubscription frm = new frmSubscription();
                        
                        frm.TipoProvento  = TipoProvento ;
                        frm.Ticker  = Ticker;
                        frm.ValorEmissao = valor;
                        frm.Percent = sPercent;
                ...
                if(TipoProvento == "GRUPAMENTO")
                {
                    ...
                    valor = (double.Parse(valor) - 1).ToString("0.0000000000000000");
                    FlagInsert = true;
                    TransType = 23;
                }
                ...
                if(TransType == 23)
                {
                    Bonus = Convert.ToDouble(valor) / PercAdjust;
                }
                ...
                if(FlagInsert == true)
                {
                    ...
                    if(IdSecurity != 0)
                    {
                        ...
                        double Lotsize = curConn.Return_Double("SELECT RoundLot FROM Tb001_Securities WHERE IdSecurity = " + IdSecurity + " ;");
                        ...
                        string SQLString = "INSERT INTO [NESTDB].[dbo].[Tb720_Dividends]([Transaction_Type],[Id_Ticker],[Declared_Date],[Ex_Date],[Record_Date],[Payment_Date],[Per_Share_Net],[Per_Share_Gross],[Percent_Bonus],[Description],IdExchangeDividend)" +
                            "VALUES (" + TransType + ", " + IdSecurity + ", '" + Data.ToString("yyyyMMdd") + "', '" + Data.ToString("yyyyMMdd") + "', '" + RecDate.ToString("yyyyMMdd") + "', '" + datapag2.ToString("yyyyMMdd") + "', " + (Net / Lotsize).ToString().Replace(",", ".") + ", " + (Gross / Lotsize).ToString().Replace(",", ".") + ", " + Bonus.ToString().Replace(",", ".") + ", '','" + Id_News + "')";
