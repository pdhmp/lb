...
namespace LiveBook
{
    public partial class frmEditFoward : LBForm
    {
        ...
        {
            LiveDLL.FormUtils.LoadCombo(this.cmbBook, "Select Id_Book, Book from dbo.Tb400_Books (nolock) ", "Id_Book", "Book", 99);
        ...
        {
            string StringSQL =
                            "Select A.Id_Foward,A.Id_Ticker,A.Id_Book,A.Id_Section, dbo.FCN_FW_Available_Quantity(a.Id_Foward, GETDATE()) AS AvailableQuantity, Quantity As InitialQuantity  ,A.Price,A.Cash,A.PriceBrokerage,A.SpotPriceBrokerage, " +
            ...
            foreach(DataRow curRow in curTable.Rows)
            {
                ...
                this.txtQuantity.Text = curRow["AvailableQuantity"].ToString();
                ...
                LiveDLL.FormUtils.LoadCombo(this.cmBroker, "Select Id_Account,Nome from VW_Account_Broker Where Id_Portfolio = " + curRow["Id_Portfolio"].ToString() + " order by Show_Prefered desc,Nome asc", "Id_Account", "Nome", 99);
        ...
        public void Carrega_Section()
        {
            ...
            try
            {
                if(curUtils.IsNumeric(cmbBook.SelectedValue))
                {
                    ...
                    //SubEstrategia
                    LiveDLL.FormUtils.LoadCombo(this.cmbSection, "Select Id_Section,Section from VW_Book_Strategies where Id_Book =" + valor, "Id_Section", "Section", 99);
                }
            }
            catch(Exception e)
            {
