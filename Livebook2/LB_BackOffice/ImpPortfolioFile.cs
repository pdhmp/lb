...
namespace LiveBook
{
    ...
    public class Mellon
    {
            ...
            while ((curLine = sr.ReadLine()) != null)
            {
                ...
                else
                {
                    if (curLine.Contains("Cliente	Data"))
                    {
                        ...
                        if (refLine.Contains("ICATU")) IdPortfolio = 55;
                        if (refLine.Contains("NEST LONG ONLY")) IdPortfolio = 80;
        ...
        void RealMellonAnalise(int IdPortfolio, string curFilePathAndName, string curFileName, Encoding currentEncoding)
        {
            ...
            if (curFileName.Contains("PREV"))
            {
                IdPortfolio = 55;
                PortName = "Icatu Nest Prev";
            }

            if (curFileName.Contains("ONLY"))
            {
                IdPortfolio = 80;
                PortName = "Nest Long Only";
            }
        ...
        int ReturnIdPortfolio(string PortName)
        {
            ...
            if (PortName.Contains("ICATU")) IdPortfolio = 55;
            if (PortName.Contains("NEST HEDGE FIC DE FIM")) IdPortfolio = 20;
            if (PortName.Contains("NEST ARB 30 FIC FIM")) IdPortfolio  = 62;

            if (PortName.Contains("NEST LONG ONLY")) IdPortfolio = 80;
        ...
        void ReadMellonAnaliseAnalitica(int IdPortfolio, string curFilePathAndName, string curFileName, Encoding currentEncoding)
        {
            ...
            while ((curLine = sr.ReadLine()) != null)
            {
                ...
                else
                {
                    ...
                    else if (curLine == "Caixa") { InsertMode = "CAA"; sr.ReadLine(); }
                    else if (curLine == "Rentabilidade") { InsertMode = "RENT"; sr.ReadLine(); }
        ...
        void ReadBtgArray(string curFileName)
        {
            ...
            for (int Count = 1; Count < BtgArray.GetLength(0) + 1; Count++)
            {
                if (BtgArray[Count, 1] != null)
                {
                    ...
                    else if (BtgArray[Count, 1].ToString() == "NAV & Shares")//
                    { InsertMode = "NavShare"; }
                    else if (BtgArray[Count, 1].ToString() == "Performance")//
                    { InsertMode = "Performance"; }
                    else if(BtgArray[Count, 1].ToString() == "NEST FUND LTD")//
                    { InsertMode = "NestFund"; }
                    else if (BtgArray[Count, 1].ToString() == "Brazilian Account")//
                    { InsertMode = "BRAccount"; }
                    else if (BtgArray[Count, 1].ToString() == "Past Due")//
                    { InsertMode = "PastDue"; }
                    else if (BtgArray[Count, 1].ToString() == "Payable / Receivable Brazil")//
                    { InsertMode = "PayRecBR"; }
                    else if (BtgArray[Count, 1].ToString() == "Payable / Receivable Offshore")//
                    { InsertMode = "PayRecUSD"; }
                    else if (BtgArray[Count, 1].ToString() == "Stock Borrowing & Lending")//
                    { InsertMode = "StockBorrLend"; }
                    else if (BtgArray[Count, 1].ToString() == "Expenses")//
                    { InsertMode = "Expenses"; }
                    else if (BtgArray[Count, 1].ToString() == "Money Markets")//
                    { InsertMode = "MoneyMkt"; }
                    else if (BtgArray[Count, 1].ToString() == "Brazilian Stocks")//
                    { InsertMode = "BRStocks"; }
                    else if (BtgArray[Count, 1].ToString() == "Brazilian Stock Options")//
                    { InsertMode = "BRStockOps"; }
                    else if (BtgArray[Count, 1].ToString() == "Stocks & ADRs")//
                    { InsertMode = "StocksADR"; }
                    else if (BtgArray[Count, 1].ToString() == "Stock & ADR Options")//
                    { InsertMode = "StocksADROps"; }
                    else if (BtgArray[Count, 1].ToString() == "Brazilian Dividends")//
                    { InsertMode = "BRDivid"; }
                    else if (BtgArray[Count, 1].ToString() == "Brazilian Bonds")//
                    { InsertMode = "BRBonds"; }
                    else if (BtgArray[Count, 1].ToString() == "Bonds")//
                    { InsertMode = "Bonds"; }
                    else if (BtgArray[Count, 1].ToString() == "Brazilian Futures")//
                    { InsertMode = "BRFuts"; }
                    else if (BtgArray[Count, 1].ToString() == "Futures")//
                    { InsertMode = "Futs"; }
                    else if (BtgArray[Count, 1].ToString() == "Future Options")//
                    { InsertMode = "FutOps"; }
                    else if (BtgArray[Count, 1].ToString() == "Funds")//
                    { InsertMode = "Funds"; }
                    else if (BtgArray[Count, 1].ToString() == "General Information")//
                    { InsertMode = "GenOps"; }
                }
            ...
            try
            {
                if (InsertString.Length > 10)
                {
                    using (newNestConn curConn = new newNestConn())
                    {
                        string exec = InsertString.Replace(",-,", ",0,");
                        result = curConn.ExecuteNonQuery(exec.Replace(",-,", ",0,"));
                        tempString = "";
    ...
    public class Itau : IDisposable
    {
        ...
        public void ImportFile(string curFilePathAndName, Sheets ItauWorksheets)
        {
            try
            {
                ...
                for (int i = 1; i < (ItauWorkbook.Worksheets.Count) + 1; i++)
                {
                    ...
                    else if (ItauWorksheets[i].Name() == "Rentabilidade") { insRentabilidade(ItauWorksheets, ItauWorkbook.Name.ToString(), refDate, i); }
        ...
        public void insRentabilidade(Sheets _ItauWorksheets, string _ItauWorkbook, DateTime _refDate, int _i)
        {
            System.Text.StringBuilder strBuilder;
            System.Data.DataTable dtSheet = ExecutarConsulta(_ItauWorksheets[_i].Name.ToString());
            ...
            if (dtSheet.Rows.Count > 0)
            {
                for (int i = 0; i <= dtSheet.Rows.Count - 1; i++)
                {
                    if (dtSheet.Rows[i] != null && dtSheet.Rows[i]["Carteira/Fundo"].ToString() != "")
                    {
                        ...
                        strBuilder.AppendFormat("INSERT [NESTIMPORT].[dbo].[Tb_Itau_Rentabilidade] SELECT ISNULL('{0}',NULL),ISNULL('{1}',null),50,ISNULL('{2}',NULL),ISNULL('{3}',NULL),ISNULL({4},NULL),ISNULL({5},NULL),ISNULL({6},NULL),ISNULL({7},NULL),ISNULL({8},NULL),ISNULL({9},NULL),ISNULL({10},NULL)",
                                                                                                _ItauWorkbook.ToString(),
                                                                                                _refDate.ToString("yyyy-MM-dd"),
                                                                                                dtSheet.Rows[i]["Carteira/Fundo"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Indexador"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Benchmark"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Rentabilidade Real"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Variação Diária"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Variação Mensal"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Variação Anual"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Últimos 6 Meses"].ToString().Replace(",", "."),
                                                                                                dtSheet.Rows[i]["Últimos 12 Meses"].ToString().Replace(",", "."));
