...
namespace LiveBook
{
    public partial class frmPositions : LBForm
    {
        ...
        private void frmPositions_Load(object sender, EventArgs e)
        {
            ...
            LiveDLL.FormUtils.LoadCombo(this.cmbChoosePortfolio, "Select Id_Portfolio,Port_Name, Id_Port_Type from  dbo.Tb002_Portfolios (nolock) where Discountinued<>1 and Id_Port_Type=1 UNION ALL SELECT '-1', 'All Portfolios', '0' ORDER BY Id_Port_Type DESC,Port_Name", "Id_Portfolio", "Port_Name", 99);
        ...
        private void cmdExcel_Click(object sender, EventArgs e)
        {
            ...
            if (fileName != "")
            {
                ...
                string fileName_Log = "T:\\Log\\Reports\\Position_Id_LB_" + LiveDLL.NUserControl.Instance.User_Id + "_Id_AD_" + user + "_Computer_" + Loca_Machine + "_Portfolio_" + cmbChoosePortfolio.Text + "_Date_" + DateTime.Now.ToString("yyyyMMdd_HH-mm-ss") + ".xls";
        ...
        private void radRealTime_CheckedChanged(object sender, EventArgs e)
        {
            if (radRealTime.Checked == true)
            {
                ...
                LiveDLL.FormUtils.LoadCombo(this.cmbChoosePortfolio, "Select Id_Portfolio,Port_Name, Id_Port_Type from  dbo.Tb002_Portfolios (nolock) where Id_Port_Type=2 and RT_Position=1 and Discountinued<>1 UNION ALL SELECT '-1', 'All Portfolios', '0' ORDER BY Id_Port_Type DESC,Port_Name", "Id_Portfolio", "Port_Name", tempPortfolio);
        ...
        private void radHistoric_CheckedChanged(object sender, EventArgs e)
        {
            if (radHistoric.Checked == true)
            {
                ...
                LiveDLL.FormUtils.LoadCombo(this.cmbChoosePortfolio, "Select Id_Portfolio,Port_Name, Id_Port_Type from  dbo.Tb002_Portfolios (nolock) where Discountinued<>1 and Id_Port_Type in (1,2) and Id_portfolio <> 48 UNION ALL SELECT '-1', 'All Portfolios', '0' ORDER BY Id_Port_Type DESC,Port_Name", "Id_Portfolio", "Port_Name", tempPortfolio);
        ...
        private void dgPositions_RowStyle(object sender, RowStyleEventArgs e)
        {
            if (!dgPositions.IsGroupRow(e.RowHandle))
            {
                ...
                {
                    if (LiveDLL.NUserControl.Instance.User_Id != 17) { e.Appearance.BackColor = Color.FromArgb(222, 254, 235); };
                }

                if (Convert.ToDouble(dgPositions.GetRowCellValue(e.RowHandle, "Column28")) < 0)
                {
                    if (LiveDLL.NUserControl.Instance.User_Id != 17) { e.Appearance.BackColor = Color.FromArgb(250, 220, 216); };
                }

                if (Convert.ToDouble(dgPositions.GetRowCellValue(e.RowHandle, "Column28")) == 0)
                {
                    if (LiveDLL.NUserControl.Instance.User_Id != 17) { e.Appearance.BackColor = Color.White; };
                }
        ...
        private void dgPositions_CustomDrawCell(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
        {
            if (e.CellValue != null)
            {
                ...
                if (LiveDLL.NUserControl.Instance.User_Id == 17)
                {
        ...
        private void dgPositions_MouseMove(object sender, MouseEventArgs e)
        {
            GridView view = sender as GridView;
            if (e.Button == MouseButtons.Left && downHitInfo != null)
            {
                ...
                if (!dragRect.Contains(new Point(e.X, e.Y)))
                {
                    if (downHitInfo.Column != null)
                    {
                        ...
                        string IdTicker = "DRAGITEM\t" + dgPositions.GetRowCellValue(downHitInfo.RowHandle, curCol.Replace("col", "")).ToString();
