...
namespace LiveBook
{
    public partial class frmInsertSecurity : Form
    {
        ...
        {
            LiveDLL.FormUtils.LoadCombo(this.cmbPriceTable, "Select Id_Price_Table, Price_Table_Name from dbo.Tb024_Price_Table", "Id_Price_Table", "Price_Table_Name", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbIssuer, "Select IdIssuer, IssuerName from Tb000_Issuers", "IdIssuer", "IssuerName", 99);
            LiveDLL.FormUtils.LoadCombo(this.cmbCurrency, "Select Id_Moeda, Currency, CASE WHEN (Id_Moeda=900 OR Id_Moeda=1042 OR Id_Moeda=929 OR Id_Moeda=4856 OR Id_Moeda=933) THEN 1 ELSE 0 END AS ForceOrder from dbo.Vw_Moedas ORDER BY ForceOrder DESC, Currency", "Id_Moeda", "Currency", 99);
            LiveDLL.FormUtils.LoadCombo(this.cmbInstrument, "Select Id_Instrumento, Instrumento from Tb029_Instrumentos", "Id_Instrumento", "Instrumento", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbAsset_Class, "Select Id_Classe_Ativo, Classe_Ativo from dbo.Tb028_Classe_Ativo", "Id_Classe_Ativo", "Classe_Ativo", 0);
        }
