...
namespace LiveBook
{
    ...
    public partial class frmEditSecurity : LBForm
    {
        ...
        {
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdIssuer, "Select IdIssuer,IssuerName from Tb000_Issuers (nolock) ", "IdIssuer", "IssuerName", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdPriceTable, "Select Id_Price_Table,Price_Table_Name from Tb024_Price_Table (nolock) ", "Id_Price_Table", "Price_Table_Name", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdCurrency, "Select Id_Moeda,Currency from dbo.Vw_Moedas (nolock) ", "Id_Moeda", "Currency", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdPremiumCurrency, "Select Id_Moeda,Currency from dbo.Vw_Moedas (nolock) ", "Id_Moeda", "Currency", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdStrikeCurrency, "Select Id_Moeda,Currency from dbo.Vw_Moedas (nolock) ", "Id_Moeda", "Currency", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdPrimaryExchange, "Select Id_Mercado, Cod_Mercado from Tb107_Mercados (nolock) ", "Id_Mercado", "Cod_Mercado", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdUnderlying, "Select IdSecurity,NestTicker from dbo.Tb001_Securities (nolock) where IdPriceTable <> 7 ORDER BY NestTicker", "IdSecurity", "NestTicker", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdInstrument, "Select Id_Instrumento,Instrumento from Tb029_Instrumentos (nolock) ", "Id_Instrumento", "Instrumento", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdAssetClass, "Select Id_Classe_Ativo,Classe_Ativo  from Tb028_Classe_Ativo (nolock) ", "Id_Classe_Ativo", "Classe_Ativo", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_HistUpdateFrequency, "Select Id_Frequencia,Frequencia from Tb105_Frequencia_Atualizacao (nolock) WHERE Id_Frequencia <> 1 ", "Id_Frequencia", "Frequencia", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_HistUpdateSource, "Select Id_Sistemas_Informacoes,Descricao  from Tb102_Sistemas_Informacoes (nolock) WHERE (Hist = 1 or RT is null)", "Id_Sistemas_Informacoes", "Descricao", 0);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_RTUpdateSource, "Select Id_Sistemas_Informacoes,Descricao  from Tb102_Sistemas_Informacoes(nolock)  WHERE(RT = 1 or RT is null)", "Id_Sistemas_Informacoes", "Descricao", 0);

            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdRegion, "Select Id_Geografia,Nome from Tb100_Geografia (nolock) order by ordem", "Id_Geografia", "Nome", 99);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdNestSector, "Select Id_Setor, Setor from Tb113_Setores (nolock) order by Setor", "Id_Setor", "Setor", 99);
            LiveDLL.FormUtils.LoadCombo(this.cmbd_GicsSubIndustry, "Select Id_Sub_Industry ,Sub_Industry from Tb205_Sub_Industry (nolock) order by Sub_Industry", "Id_Sub_Industry", "Sub_Industry", 99);
        ...
        {
            // if(txtsearch.Text == "") txtsearch.Text = "VALE5";
            if (txtsearch.Text == "") txtsearch.Text = "VALE5";
            ...
            if (txtsearch.Text.Trim() != "")
            {
                ...
                for (int f = 0; f < strFields.Length; f++)
                {
                    ...
                    {
                        strWhere = strWhere +
                            "AND (A.NestTicker like '%" + strFields[f] +
                            "%' OR A.SecName Like '%" + strFields[f] +
                            "%' OR A.ExchangeTicker Like '%" + strFields[f] +
                            "%' OR ReutersTicker Like '%" + strFields[f] +
                            "%' OR BloombergTicker Like '%" + strFields[f] +
                            "%' OR Imagineticker like '%" + strFields[f] +
                            "%' OR CyrnelTicker like '%" + strFields[f] +
                            "%' OR Btgticker like '%" + strFields[f] +
                            "%' OR Itauticker like '%" + strFields[f] +
                            "%' OR Mellonticker like '%" + strFields[f] +
                            "%' OR Adminticker like '%" + strFields[f] +
                            "%' OR convert(varchar,IdSecurity)='" + strFields[f] + "')";
                    }
                }
            }

            int tempIdSecurity = 0;

            if (int.TryParse(strFields[0], out tempIdSecurity))
            {
                strWhere += " OR IdSecurity = " + strFields[0];
            }
        ...
        {
            if (tabCtrSecurities.SelectedTab.Name == "tabPrices")
            {
                string SQLString;
        ...
        private void Carrega_Ativos(int Id_Ativo)
        {
            ...
            if (Convert.ToDateTime(dtpd_Expiration.Value) < DateTime.Now.Subtract(new TimeSpan(20, 0, 0, 0)) && LiveDLL.NUserControl.Instance.User_Id != 23 && dtpd_Expiration.Value != Convert.ToDateTime("1/1/1900"))
            {
        ...
        void ReloadlstAvDates()
        {
            ...
            LiveDLL.FormUtils.LoadList(lstAvDates, "SELECT ValidAsOfDate FROM NESTDB.dbo.Tb001_Securities_Variable (nolock) WHERE IdSecurity=" + lbld_IdSecurity.Text, "ValidAsOfDate", "ValidAsOfDate");
            this.lstAvDates.SelectedIndexChanged += new System.EventHandler(this.lstAvDates_SelectedIndexChanged);
        ...
        {
            if (txtd_CyrnelTicker.Text == "")
            {
                if (Convert.ToInt32(MessageBox.Show("Cyrnel Ticker nulo, continuar assim mesmo?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question)) != 6)
                {
                    return;
                }
            }
        ...
        private int Atualiza_Dados()
        {
            ...
            if (lbld_IdSecurity.Text != "" && lstAvDates.SelectedValue != null && lstAvDates.SelectedValue.ToString() != "")
            {
                SQLString = "UPDATE [Tb001_Securities_Fixed] " + // Tabela alterade de Fixed para Fixed_teste
                            ...
                            "  ,[CyrnelTicker] = '" + txtd_CyrnelTicker.Text.ToString() + "'" +
                            ...
                            "  ,[Adminticker] = '" + txtd_AdminTicker.Text.ToString() + "'" +
            ...
            if (retorno != 0)
            {
                ...
                LiveDLL.FormUtils.LoadCombo(this.cmbd_IdIssuer, "Select IdIssuer, IssuerName from Tb000_Issuers", "IdIssuer", "IssuerName", int.Parse(cmbd_IdIssuer.SelectedValue.ToString()));
            }
            return retorno;
        }
        ...
        {
            int IdSecurity;
            if (int.TryParse(this.lbld_IdSecurity.Text, out IdSecurity))
            {
        ...
        private void lblNewIssuer_Click(object sender, EventArgs e)
        {
            ...
            LiveDLL.FormUtils.LoadCombo(this.cmbd_IdIssuer, "Select IdIssuer, IssuerName from Tb000_Issuers", "IdIssuer", "IssuerName", 99);
        }
        ...
        void Carrega_Combo_Fontes(int Id_Ticker)
        {
            ...
            LiveDLL.FormUtils.LoadCombo(cmbSource, "SELECT -1 AS Id_Sistemas_Informacoes, 'All' AS Descricao UNION ALL SELECT Id_Sistemas_Informacoes, Descricao from dbo.Tb102_Sistemas_Informacoes A (nolock) INNER JOIN " + PriceTableName + " B ON A.Id_Sistemas_Informacoes=B.Source WHERE IdSecurity=" + Id_Ticker + " GROUP BY A.Id_Sistemas_Informacoes, A.Descricao ORDER BY Descricao", "Id_Sistemas_Informacoes", "Descricao", 1);
        }
        ...
        void Carrega_Combo_Tipos(int Id_Ticker)
        {
            ...
            if (curSource != -1)
            {
                LiveDLL.FormUtils.LoadCombo(cmbPrice_Type, "SELECT -1 AS SrType, 'All' AS Preco UNION ALL SELECT A.Id_Tipo_Preco as SrType, A.Preco FROM dbo.Tb116_Tipo_Preco A (nolock)  INNER JOIN " + PriceTableName + " B ON A.Id_Tipo_Preco=B.SrType WHERE IdSecurity=" + Id_Ticker + " AND Source=" + Id_Source + " GROUP BY A.Id_Tipo_Preco, A.Preco ORDER BY Preco", "SrType", "Preco", 1);
            }
            else
            {
                LiveDLL.FormUtils.LoadCombo(cmbPrice_Type, "SELECT -1 AS SrType, 'All' AS Preco UNION ALL SELECT A.Id_Tipo_Preco as SrType, A.Preco FROM dbo.Tb116_Tipo_Preco A (nolock)  INNER JOIN " + PriceTableName + " B ON A.Id_Tipo_Preco=B.SrType WHERE IdSecurity=" + Id_Ticker + " GROUP BY A.Id_Tipo_Preco, A.Preco ORDER BY Preco", "SrType", "Preco", 1);
            }
            cmbPrice_Type.SelectedIndexChanged += new System.EventHandler(cmbPrice_Type_SelectedIndexChanged);
            Carrega_Grid_Prices();
        }
        ...
        private void cmdInsertPrice_Click(object sender, EventArgs e)
        {
            ...
            insPrice.SingleSecMode = int.Parse(parte[0]);
