...
namespace LiveBook
{
    public class LB_Utils
    {
        ...
        public void SetColumnStyle(DevExpress.XtraGrid.Views.Grid.GridView GridName, int Type_Grid, string Coluna)
        {
            int IdGrid;
            string ColumnName;
            int largura;
            ...
            try
            {
                ...
                foreach (DevExpress.XtraGrid.Columns.GridColumn col in GridName.Columns)
                {
                    GridName.Columns[c].Caption = col.ToString();//.Replace(' ','\n');
                    c++;
                }


                string SQLString = "Select IdGrid from UI_DEFS.dbo.Tb210_Grids (nolock) where GridName= '" + GridName.Name.ToString() + "'";
                using (newNestConn curConn = new newNestConn())
                {
                    IdGrid = Convert.ToInt32(curConn.Execute_Query_String(SQLString));
                    SQLString = "Select * from UI_DEFS.dbo.Tb211_Grid_Columns (nolock) where IdGrid =" + IdGrid + " and IdUser =" + LiveDLL.NUserControl.Instance.User_Id + " order by GroupIndex ASC, VisibleIndex ASC";
                    ...
                    foreach (DataRow curRow in curTable.Rows)
                    {
                        ...
                        ColumnName = Convert.ToString(curRow["ColumnName"]);
                        //                        Caption = Convert.ToString(curRow["Caption_Coluna"]);

                        DateTime tempDate;

                        if (ColumnName != "Edit" && ColumnName != "Cancel" && ColumnName != "" && !DateTime.TryParse(ColumnName.Replace("col", ""), out tempDate))
                        {
                            largura = Convert.ToInt32(curRow["ColumnWidth"]);
                            visivel = Convert.ToBoolean(curRow["isVisible"]);
                            Indice = Convert.ToInt32(curRow["VisibleIndex"]);
                            indice_grupo = Convert.ToInt32(curRow["GroupIndex"]);
                            //                            Caption = curRow["Caption_Coluna"].ToString();

                            if (Type_Grid == 1)
                            {
                                SetColumnStyle_RT(GridName, ColumnByCaption(ColumnName.ToString(), GridName), visivel, largura, Indice, indice_grupo, ColumnName);
                            }
                            if (Type_Grid == 2)
                            {
                                SetColumnStyle_Report(GridName, ColumnByCaption(ColumnName.ToString(), GridName), visivel, largura, Indice, indice_grupo, Coluna);
                            }
                            if (Type_Grid == 3)
                            {
                                SetColumnStyle_Diferences(GridName, ColumnByCaption(ColumnName.ToString(), GridName), visivel, largura, Indice, indice_grupo, Coluna);
                            }
            ...
            {
                Log_Error_Dump_TXT(e.ToString(), this.ToString());
            }
        }
        ...
        public bool IsNumeric(object value)
        {
            ...
            {
                decimal curValue;
                if (decimal.TryParse(value.ToString(), out curValue))
                {
                    return true;
                }
                else
        ...
        public void SetColumnStyle_RT(DevExpress.XtraGrid.Views.Grid.GridView Nome_Grid, DevExpress.XtraGrid.Columns.GridColumn c, Boolean visivel, int largura, int Indice, int indice_grupo, string Caption)
        {
            ...
            if (Nome_Grid.Name.Contains("dgPos"))
            { }
        ...
        public void SetColumnStyle_Diferences(DevExpress.XtraGrid.Views.Grid.GridView Nome_Grid, DevExpress.XtraGrid.Columns.GridColumn c, Boolean visivel, int largura, int Indice, int indice_grupo, string Caption)
        {
            if (c == null) return;
            if (Nome_Grid.Columns.Contains(Nome_Grid.Columns[c.FieldName]) == true)
            {
            ...
            cn = new StyleFormatCondition(FormatConditionEnum.Greater, Nome_Grid.Columns[Caption], null, -1);
            cn.Appearance.ForeColor = Color.Green;
            cn.ApplyToRow = true;
            Nome_Grid.FormatConditions.Add(cn);


            cn = new StyleFormatCondition(FormatConditionEnum.Less, Nome_Grid.Columns[Caption], null, 1);
            cn.Appearance.ForeColor = Color.Green;
            cn.ApplyToRow = true;
            Nome_Grid.FormatConditions.Add(cn);
        ...
        public void SetColumnStyle_Report(DevExpress.XtraGrid.Views.Grid.GridView Nome_Grid, DevExpress.XtraGrid.Columns.GridColumn c, Boolean visivel, int largura, int Indice, int indice_grupo, string Coluna)
        {
            ...
            {
                if (Nome_Grid.Columns.Contains(Nome_Grid.Columns[c.Caption.Replace(" ", "")]) == true)
                {
                    Nome_Grid.Columns[c.Caption.Replace(" ", "")].Visible = visivel;
                    Nome_Grid.Columns[c.Caption.Replace(" ", "")].VisibleIndex = Indice;
                    Nome_Grid.Columns[c.Caption.Replace(" ", "")].Width = largura;
                    Nome_Grid.Columns[c.Caption.Replace(" ", "")].GroupIndex = indice_grupo;
                    //Nome_Grid.Columns[c.Caption].Fixed = 
                }
            }
            StyleFormatCondition cn;
        ...
        public int Save_Columns(DevExpress.XtraGrid.Views.Grid.GridView Nome_Grid)
        {
            ...
            using (newNestConn curConn = new newNestConn())
            {
                Id_Grid = Convert.ToInt32(curConn.Execute_Query_String("Select IdGrid from UI_DEFS.dbo.Tb210_Grids (nolock) where GridName= '" + Nome_Grid.Name + "'"));
                ...
                foreach (DevExpress.XtraGrid.Columns.GridColumn coluna in Nome_Grid.Columns)
                {
                    ...
                    if (Nome != "Edit" && Nome != "Cancel" && Nome != "")
                    {
                        ...
                        indice = Nome_Grid.Columns[f].VisibleIndex;
                        indice_grupo = Nome_Grid.Columns[f].GroupIndex;
                        ...
                        SQLString = " INSERT INTO [UI_DEFS].[dbo].[Tb211_Grid_Columns] " +
                       " ([IdGrid],[IdUser],[ColumnName],[ColumnWidth],[isVisible],[VisibleIndex],[GroupIndex])" +
                       " VALUES (" + Id_Grid + "," + LiveDLL.NUserControl.Instance.User_Id + ",'" + Nome + "'," + largura + "," + visivel + "," + indice + "," + indice_grupo + ")";
                        ...
                        SQLString = "INSERT INTO Tb109_Caractisticas_Colunas([Id_Grid], [Id_User], [Nome_Coluna], [Largura], [Visible], [Indice],Indice_Grupo,Caption_Coluna,Versao)" +
                                    " VALUES(" + Id_Grid + "," + LiveDLL.NUserControl.Instance.User_Id + ",'" + Nome + "'," + largura + "," + visivel + "," + indice + "," + indice_grupo + ",'" + Caption + "',2)";
                        */
        ...
        public Boolean Verifica_Acesso(int Id_Grupo)
        {
            try
            {
                using (newNestConn curConn = new newNestConn())
                {
                    string SQLString = "SELECT COUNT(*) FROM NESTDB.dbo.FCN_CheckUserAccess(" + LiveDLL.NUserControl.Instance.User_Id + ") WHERE IdGroup IN (" + Id_Grupo.ToString() + ")";
        ...
        public string Retorna_ordem(int Grid_Cod)
        {
            ...
            using (newNestConn curConn = new newNestConn())
            {
                SQLString = "Select Campos_Mostrar from Tb109_Ordens_Colunas where Id_Usuario =" + LiveDLL.NUserControl.Instance.User_Id + " and Grid_Cod = " + Grid_Cod;
                SQLString = curConn.Execute_Query_String(SQLString);
            }
            return SQLString;
        }
        //#Fora de USO
        ...
        public string Retorna_Tabela_Preco_Trade(int Id_Ativo, int tipo_tabela)
        {
            ...
            {
                string Nome_Tabela = "";
                int IdPriceTable = Convert.ToInt32(curConn.Execute_Query_String("Select IdPriceTable from dbo.Tb001_Securities (nolock)  where IdSecurity = " + Id_Ativo));
        ...
        public void SaveFormProperties(Form FormToSave, int Id_User)
        {
            ...
            using (LiveDLL.old_Conn curConn = new LiveDLL.old_Conn())
            {
                ...
                SQLString = "SELECT Id_Form FROM UI_DEFS.dbo.Tb201_Forms (nolock) WHERE Form_Name= '" + FormName + "' AND IdApplication=" + IdApplication;
                try
                {
            ...
            if (FormToSave.WindowState.ToString() != "Maximized")
            {
                ...
                {
                    if (FormName == "frmPerfChart") FormCaption = "Intraday Performance";
        ...
        public bool LoadFormProperties(Form FormToLoad, int Id_User, string FormCaption)
        {
            ...
            using (LiveDLL.old_Conn curConn = new LiveDLL.old_Conn())
            {
                ...
                SQLString = "SELECT Id_Form FROM UI_DEFS.dbo.Tb201_Forms(nolock)  WHERE Form_Name= '" + FormName + "' AND IdApplication=" + IdApplication.ToString(); ;
                ...
                SQLString = "SELECT * FROM UI_DEFS.dbo.Tb201_Form_Properties (nolock) WHERE Id_Form=" + Id_Form + " AND Id_User=" + Id_User + " AND Form_Caption='" + FormCaption + "'";
        ...
        public void Set_Controls_Form(Form Formulario)
        {
            ...
            {
                SQLString = "SELECT Id_Form FROM UI_DEFS.dbo.Tb201_Forms (nolock) WHERE Form_Name= '" + Formulario.Name + "' AND IdApplication=" + IdApplication;

                Id_Form = Convert.ToInt32(curConn.Execute_Query_String(SQLString));

                SQLString = "SELECT * from UI_DEFS.dbo.Tb201_Form_Properties (nolock) WHERE Id_Form=" + Id_Form + " AND Id_User=" + LiveDLL.NUserControl.Instance.User_Id;
                        ...
                        if (ControlValues.Trim() != "")
                        {
                            ...
                            for (int i = 0; i < curControlData.Length; i++)
                            {
                                if (curControlData[i].Trim() != "")
                                {
                                    ...
                                    {
                                        if (((ComboBox)curControl).Items.Count > Convert.ToInt32(curVal))
                                        {
                                            ((ComboBox)curControl).SelectedIndex = Convert.ToInt32(curVal);
                                        }
                                    }
                                }
                            }
        ...
        public int Log_Error_Dump_TXT(string MsgError, string Source)
        {
            string user = Environment.UserName.ToString();
            string Loca_Machine = Environment.MachineName.ToString();

            string strPath = @"C:\Log\LiveBook\Error_" + user + "_" + Loca_Machine + ".txt";
        ...
        public int Log_Open_Close_Form(string FormName, int ActionType)
        {
            string user = Environment.UserName.ToString();
            string Loca_Machine = Environment.MachineName.ToString();
            string StringType = "";
            ...
            string strPath = @"C:\Log\LiveBook\StatusForm_" + user + "_" + Loca_Machine + ".txt";
        ...
        public static void carregacombo_Table(ComboBox ComboName, DataTable table, string Value, string Display, int selected)
        {
